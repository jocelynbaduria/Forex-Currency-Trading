...
from src.methods import train_model, evaluate_model, train_pg_model
...
def run(training_stock, validation_stock, window_size, batch_size, episode_count, model_type="ddqn", model_name = None, pretrained = False, verbose = False):
  ...
  if model_type == "ddqn":
    ...
    print("Using policy gradient model")
    agent = PGAgent(num_features, 3)
  else:
  ...
  for episode in range(1, episode_count + 1):
    ...
    if(model_type == "ddqn"):
      ...
      training_result = train_pg_model(agent, episode, training_data, episode_count = episode_count, batch_size = batch_size, window_size = window_size)
    else:
      print("Invalid model type!!")
